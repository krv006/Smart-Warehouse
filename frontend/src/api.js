...
export const api = {
  ...
  pendingChanges: (params = {}) => request(`/common/pending-changes/${toQuery({ page_size: 50, ...params })}`),
  pendingChangeApprove: (id) => request(`/common/pending-changes/${id}/approve/`, { method: 'POST' }),
  pendingChangeReject: (id, reviewNote) => request(`/common/pending-changes/${id}/reject/`, { method: 'POST', body: JSON.stringify({ review_note: reviewNote }) }),
  ...
  bookings: (params = {}) => request(`/orders/booking/${toQuery({ page_size: 50, ...params })}`),
