...
export default function ListFiltersPanel({ title, filters, onChange }) {
  ...
  const clearAll = useCallback(() => {
    onChange({ status: '', client: '', category: '', import_type: '', date_from: '', date_to: '' })
  }, [onChange])

  if (!features.status && !features.client && !features.date && !features.category && !features.importType) return null
