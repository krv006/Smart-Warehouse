...
| `stocks(params)` | GET | `/warehouse/stocks/` | `page_size=30`, `product`, `warehouse_location`, `status` (`in_stock`\|`low_stock`\|`out_of_stock`\|`on_the_way`), `date_from`, `date_to`, `search`, `ordering` (`category` filtri olib tashlangan) |
...
**Grid sahifalar** (`GRID_PAGES` — `App.jsx`): Mijozlar, Sotuvlar, Kirim (ilgari «Import»), Ombor, Qoldiqlar, Xarajatlar. **Kassa** va **Buyurtmalar** alohida: `KassaPage` (`/moliya/kassa`), `BuyurtmalarPage` (`/invoices/`).
...
**Bildirishnoma:** yangi bron yaratilganda barcha faol Managementga `Notification` yuboriladi (`Notification.notify_new_booking`) — `Notification` modelida yangi `booking` FK (nullable) shu bronga ishora qiladi (item 8). Frontendda `Bildirishnomalar` ro'yxatida `row.booking` bo'lgan yozuvni bosish `Bron` sahifasiga o'tkazadi (`navigateToPath(pathForPage('Bron'))`, `App.jsx`ning `ResourcePage`idagi maxsus holat) — bu hozircha **ro'yxat sahifasiga** olib boradi (aynan o'sha bron qatoriga avtomatik skroll/filtr qilinmaydi), Admin keyin ro'yxatda qidirib topadi.
